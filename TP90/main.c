...
void lireCommande(char *N){
	...
	printf("%s",nom);
	/*
	ficFacture=fopen("factures","r");
	
	
	fprintf(ficFacture,nom);


	strcpy(ficFacture,"./factures");
...
/*	do{
		fscanf(N, "%d %d", &numArticle, &qte);

		//lire et comp pour avoir les prix
		produit=fopen("","r");

		fprintf(ficFacture, "%d %s %f=%f", &qte, &nomArticle, &prixUnit, &(prixUnit*qte));
	}while (getchar() != EOF)*/
